 ...
 * <p>
 * This view isn't all that useful on its own; rather it serves as an example of how to correctly implement these types
 * of gestures to perform zooming and scrolling with interesting content types.
 * <p>
 * The view is interactive in that it can be zoomed and panned using typical <a
 * href="http://developer.android.com/design/patterns/gestures.html">gestures</a> such as double-touch, drag,
 * pinch-open, and pinch-close. This is done using the {@link ScaleGestureDetector}, {@link GestureDetector}, and
 * {@link OverScroller} classes. Note that the platform-provided view scrolling behavior (e.g.
 * {@link View#scrollBy(int, int)} is NOT used.
 * <p>
 * The view also demonstrates the correct use of <a
 * href="http://developer.android.com/design/style/touch-feedback.html">touch feedback</a> to indicate to users that
 * they've reached the content edges after a pan or fling gesture. This is done using the {@link EdgeEffectCompat}
 * class.
 * <p>
 * Finally, this class demonstrates the basics of creating a custom view, including support for custom attributes (see
 * the constructors), a simple implementation for {@link #onMeasure(int, int)}, an implementation for
 * {@link #onSaveInstanceState()} and a fairly straightforward {@link Canvas}-based rendering implementation in
 * {@link #onDraw(android.graphics.Canvas)}.
 * <p>
 * Note that this view doesn't automatically support directional navigation or other accessibility methods. Activities
 * using this view should generally provide alternate navigation controls. Activities using this view should also
 * present an alternate, text-based representation of this view's content for vision-impaired users.
 */
public class InteractiveLineGraphView extends View {
	...
	// コンストラクタ Viewを承継したクラスを自分で作成する際には必要.
	 ...
	 */
	// ペイント（Canvasに文字や線を描く際の色などを管理する筆のようなもの）を初期化
	...
	@Override
	// このカスタムView(つまりInteractiveLineGraphView）のサイズが変更された際に呼ばれるコールバック関数.画面回転時などに呼ばれる.
	...
	@Override
	// *重要* このカスタムViewのサイズが必要な時に呼ばれるコールバック関数.頻繁に呼ばれる.
	// 参考：http://qiita.com/naodroid/items/d685a0113342edbb7587
	...
	@Override
	// *重要* このカスタムViewを描画するコールバック関数.頻繁に呼ばれる.invalidate()で呼ばれる.
	 ...
	 */
	// *重要* 端までスクロールした際に表示される青いエフェクト。それ以上スクロールできないことを表示する.
	 ...
	 * given pixel coordinates, if that pixel is within the chart region described by {@link #mContentRect}. If the
	...
	private final GestureDetector.SimpleOnGestureListener mGestureListener = new GestureDetector.SimpleOnGestureListener() {
		...
		public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
			 ...
			 * additional information about the viewport, see the comments for {@link mCurrentViewport}.
			 */
